...
(function (PhotosOfUs) {
    angular.module('app')
        .controller('CartCtrl', ['$scope', '$rootScope', '$http', '$mdDialog', ($scope, $rootScope, $http, $mdDialog) => {
            ...
            const selectedItems = cart ? JSON.parse(cart) : [];
            $scope.select = (photoId, printName, quantity, price) => {
                if (!quantity) {
                    quantity = 1;
                }
                if (selectedItems.find(x => x.photoId === photoId && x.printType.type === print.name)) {
                    ...
                    selectedItems.push({
                        photoId: photoId,
                        printType: new PhotosOfUs.PrintType({ type: printName, id: null, baseCost: null }),
                        quantity: quantity,
                        id: null,
                        orderId: null,
                        unitPrice: price
            ...
            $scope.isSelected = (name) => selectedItems.find(x => x.printType.type === name);
            $scope.addToCart = () => {
