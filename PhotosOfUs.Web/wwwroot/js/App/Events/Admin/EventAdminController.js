var PhotosOfUs;
(function (PhotosOfUs) {
    angular.module('app').controller('EventAdminCtrl', ['$scope', '$rootScope', '$mdDialog', 'FolderApiClient', '$http',
        ($scope, $rootScope, $mdDialog, FolderApiClient, $http) => {
            ...
            $scope.$on('FolderRenamed', function (e, folder) {
                const index = $scope.folders.findIndex(f => f.Id === folder.Id);
                $scope.folders[index] = folder;
            });
            $scope.$on('FolderRemoved', function (e, folderId) {
                const index = $scope.folders.findIndex(f => f.Id === folderId);
...
//# sourceMappingURL=EventAdminController.js.map